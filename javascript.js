...
submit.addEventListener("click", function(event) {
    console.log("clicked");
        card.innerHTML = `<div class="flex items-center justify-center w-56 h-56 bg-transparent rounded-lg">
        <div role="status">
...
function sendEmail(event){
    ...
    var params = {
        ...
        email: document.getElementById("helper-text").value,
        message: document.getElementById("message").value
    };
    console.log(params);
    const serviceID = "service_prsmhcx";
    const templateID = "template_bkxo11k"

    emailjs.send(serviceID, templateID, params).then(response => {
        document.getElementById("helper-text").value = "";
